...
/* 

 {
        ...
        effort: "Strenuous",
        img: "Washington-StoryImage_2.jpg",
        desc: "Mt. Washington (6,288 feet) is the highest peak east of the Mississippi River and north of the Carolinas. The upper part of the mountain has aclimate similar to that of northern Labrador and supports a variety of alpine flora and fauna.",
        coords: {
            lat: 44.270403,
            lng: -71.303501
        }
...
// creates new object that reshapes mountainsArray (for easier accessibility)
function createMountainsObject(mountainsArray) {
    var mountainsObject = {};
    var mountain;

    // loop through each object in mountainsArray,  create new identical object that has each mountain as key
            /* 
                    mountain: {
                        LocationID: "ABLI",
            ...
            */
    for (mountain in mountainsArray) {
        currentMountain = mountainsArray[mountain].name; // gets the mountain of current mountain in mountainsArray

        console.log(mountain)

        mountainsObject[currentMountain] = mountainsArray[mountain]; // add currentParkState as key and its associated park info as value in new object 

        // console.log("STATE: ", nationalParksObject[currentParkState], "OBJECT OF STATE:", mountainsArray[park])
    }

    // console.log("NEWLY CREATED NATIONAL PARKS OBJECT", nationalParksObject)

    // return the newly created object so it can be used in nationalparks.js
    return mountainsObject;
}


// populate dropdown menu with mountainsArray items
function populateMountainsMenu(mountainsObject) {
    var $dropdownList3 = $('#dropdown-list3');

     // loop through locationsArray and create dropdown menu element
     for (mountain in mountainsObject) {
        currentMountain = mountainsObject[mountain].name;
        // append each location from locations array to the drop down menu
        $dropdownList3.append(`
            <li class="dropdown-item">${currentMountain}</li>
        `);
    }
 }
...
window.mountains = { 
    ...
    createMountainsObject: createMountainsObject,
    populateMountainsMenu: populateMountainsMenu,
